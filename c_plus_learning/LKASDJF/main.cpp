...
direct3d9 error: SetThumbNailClip failed: 0x800706f4
这应当是我们忽略了PCR导致的，不过使用android（
版本8.1.0，手头没有其他版本的android机测试）和ios手机浏览器或视频播放器可以正常播放。
...
int MakeTable(unsigned int *crc32_table)//制作crc校验
{
...
unsigned char* PatPacket() {
    ...
    *(p_pat + temp_len) = 0x00;
    temp_len++;
    /*固定4bit（1011），2bit必定为00，后10bit表示后续长度(9+4*PMT表数，
     * 注意其中也包括了crc_32校验码的长度，我们的节目只有视频所以这个长度
     * 为13:0x000d)，共计2字节*/
    ...
    *(unsigned short*)(p_pat + temp_len) = htons(0x0000);/*传输流ID,用户自定义（那就随便定义个0x0000）*/
    temp_len += 2;
    /*接下来2bit固定（11），5bit（00000,一旦PAT有变化，版本号加1),
     * 1bit(1,表示传送的PAT当前可以使用，若为0表示下一个表有效)*/
...
int H264ToTs()
{
    char file_in[] = "F:/c_plus_learning/LKASDJF/stream.h264";
    OpenBitstreamFile(file_in);
    MakeTable(crc32Table);
    char file_out[] = "F:/c_plus_learning/LKASDJF/stream.ts";
    OpenWriteFile(file_out);
    unsigned char* p_pat_res = PatPacket(); // 生成pat表
    write2file(p_pat_res, TS_PACKET_LEN);
    fflush(fw);
    unsigned char* p_pmt_res = PmtPacket(); //生成ptm表
    write2file(p_pmt_res, TS_PACKET_LEN);
    /*fwrite函数是在程序结束后将缓冲区的数据写入文件，
     * fflush函数确定立刻将数据写入文件*/
    fflush(fw);
